...
public function messages($messages, $options = "")
{
	...
	foreach ($messages as $id => $message) {
		...
		if (is_array($message)) $this->message(vsprintf(T("message.$id", ""), $message), $options);
		else $this->message(T("message.$message", $message), $options);
	}
}
...
public function head()
{
	...
	foreach ($this->cssFiles as $key => $files) {
		...
		if (count($files) > 0 and C("esoTalk.aggregateCSS"))
	...
	$isOperaMini4x = preg_match("/^Opera\/.+Opera Mini\/4./i", $userAgent, $userAgentMatch);
