...
class ETPlugin_Ignore extends ETPlugin {
	...
	public function handler_memberController_initProfile($sender, &$member, $panes, $controls, $actions)
	{
		...
		if ($member["ignored"]) $member["username"] = $member["username"] . "  " . T("[ignored]"); 
	}
	...
	public function action_memberController_ignore($controller, $memberId = "")
	{
		...
		$controller->redirect(URL(R("return", "member/".$memberId)));
	}
	...
	public function handler_postModel_getPostsAfter($sender, &$posts)
	{
		...
		if (count($ignoredIds)) {
			foreach ($posts as $k => &$post) {
				...
				if (in_array($post["memberId"], $ignoredIds)) {
	...
	public function handler_activityModel_afterGetResults($sender, &$result)
	{
		...
		if (count($ignoredIds)) {
			if (isset($result["fromMemberId"]) && in_array($result["fromMemberId"], $ignoredIds)) {
				if (isset($result["type"])) unset($result["type"]);
			}
		}
	}
